import { cp, lstat, mkdir, readdir, realpath, rm, rmdir } from 'node:fs/promises';
...
async function assertSafeOutputPaths(paths, { workflow, skills }) {
...
export async function install({ target, force = false, skillsOnly = false, initOnly = false, homeDir } = {}) {
  ...
  const paths = await targetPaths(target, homeDir);
  await assertSafeOutputPaths(paths, { workflow: !skillsOnly, skills: !initOnly });
  if (!skillsOnly) {
