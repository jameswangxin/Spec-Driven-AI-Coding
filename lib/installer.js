...
async function directoryPresent(path) {
  try {
...
export async function inspect({ target, homeDir, targetPlatform = 'codex' } = {}) {
  ...
  return {
    ...
    missing: [
      ...
      ...(await Promise.all(docsScaffold.map((path) => directoryPresent(join(paths.base, path))))).flatMap((installed, index) => (
...
export async function install({ target, force = false, skillsOnly = false, initOnly = false, homeDir, targetPlatform = 'codex', withClaudeMd = false } = {}) {
  ...
  if (withClaudeMd) {
    ...
    if (!(await present(guide))) await writeFile(guide, '# Project workflow\n\n@.workflow/project.md\n@.workflow/current.md\n\nUse `.workflow/` as the governance entry point. Use `docs/changes/` as the source of truth for change packages, and `docs/specs/` for currently effective behavior specs.\n');
  }
