...
function validateRequirementSchema(record, diagnostics) {
  ...
  const historyOk = Array.isArray(value.history) && value.history.length > 0 && value.history.every((entry) => entry && typeof entry === 'object' && isDate(entry.date) && (typeof entry.from === 'string' || entry.from === null) && typeof entry.to === 'string' && entry.to.length > 0 && typeof entry.note === 'string' && entry.note.trim().length > 0);
  const valid = /^REQ-\d{4}$/.test(value.id) && typeof value.title === 'string' && value.title.trim() && REQUIREMENT_STATUSES.has(value.status) && COMPLEXITIES.has(value.complexity) && Array.isArray(value.risk_tags) && value.risk_tags.every((tag) => RISK_TAGS.has(tag)) && new Set(value.risk_tags).size === value.risk_tags.length && typeof value.plan_required === 'boolean' && typeof value.plan_reason === 'string' && value.plan_reason.trim() && isDate(value.created_at) && isDate(value.updated_at) && isStringArray(value.references) && isStringArray(value.capabilities) && value.capabilities.every((cap) => /^CAP-\d{4}$/.test(cap)) && historyOk;
