...
3. **Clarify and confirm**
   ...
   - Put unresolved material questions in **待确认问题** and ask the user. Keep the requirement `draft` until they answer; do not infer approval from silence, a partial answer, or an implementation request.
...
4. **Validate and sync**
   - After modifying any `.workflow/` file, run `workflow-template --validate`; fix validation failures before continuing.
